...
\title[CEE 260/MIE 273 M3d: Binomial Dist.]{{\normalsize CEE 260/MIE 273: Probability and Statistics in Civil Engineering} \\
Lecture M3d: The  Binomial Distribution}
...
\begin{frame}
  ...
  40\% of the students in a university are engineering majors. If four students (A, B, C, D) are chosen at random, what is the probability that exactly one student will be an engineering major?
  ...
  \begin{exampleblock}{Solution}
    ...
    In one scenario, student $A$ is an engineering major, while the other 3 are not.
    \begin{eqnarray*}
     && P(A = \text{engineering}, B= \text{not}, C=\text{not}, D=\text{not}) 
      \\ \pause
&=&     P(A = \text{engineering}) \times P( B= \text{not}) \times P(C=\text{not}) \times P(D=\text{not})   \\ \pause
    ...
    {\rd \bf No.} There are 3 others: each of the students $B$, $C$ or $D$ could also be the engineering major. Thus, the total required probability is \pause     $ 4 \times (0.4)(0.6^{3}) = \pause 0.346$
  \end{exampleblock}
\end{frame}
...
\begin{frame}
  ...
  40\% of the students in a university are engineering majors. If four students (A, B, C, D) are chosen at random, now find
  the probability that exactly two students will be an engineering major?
  ...
  \begin{exampleblock}{Solution (cont.)}
    ...
    \begin{itemize}[<+->]
    \item Scenario 1: $A =$ engineering, $ B = $ engineering; $C, D = $ not
    \item Scenario 2: $A =$ engineering, $C =$  engineering; $ B, D =$ not
    \item Scenario 3: $A =$ engineering, $D =$ engineering; $B, C =$ not
    \item Scenario 4: $B =$ engineering, $C =$ engineering, $A, D=$ not
    \item Scenario 5: $B =$ engineering, $D =$ engineering, $A, C=$ not
    \item Scenario 6: $C =$ engineering, $D =$ engineering, $A, B=$ not
    \end{itemize}
    ...
    The probability of having 2 eng. majors in a random group of 4 students is: \pause
    \vspace{-2ex}
    \begin{equation*}
...
\begin{frame}
  ...
  \item $B()$ is the typical notation for a binomial distribution
  \end{itemize}
  \end{block}
  \end{frame}
...
\begin{frame}
  ...
  The probability that a grader will malfunction within 900 hrs is 0.0594.\pause
  Assuming statistical independence among the
    ...
    will malfunction in less than 900 hr of operation.
  \end{minipage}\pause
...
\begin{frame}
  ...
  \pause
    ...
    \begin{minipage}{.5\linewidth}
  Five road graders are used in the construction of a highway project.
  The probability that a grader will malfunction within 900 hrs is 0.0594.
  Assuming statistical independence among the
    conditions of the machines, evaluate the probability \textbf{no more than two} of the five machines
    will malfunction in less than 900 hr of operation.
  \end{minipage}\pause
  \begin{minipage}{.45\linewidth}
  \begin{center}
...
\end{minipage}
  \medskip
    ...
    \begin{eqnarray*}
      P(X \le x) &=& F_{X}(2) = \pause \sum_{k=0}^{2}{n \choose k}p^k (1-p)^{n-k}  \\\pause
      P(X\le 2) &=&  {5\choose 0}0.0594^0(0.9406)^{5} \pause + {5\choose 1}0.0594^1(0.9406)^{4} \pause
                    + {5\choose 2}0.0594^2(0.9406)^{3} \\ \pause
...
\end{frame}
% \begin{frame}
%   \frametitle{CDF of binomial distribution (cont.)}
%   \pause
...
\begin{frame}
  ...
  \begin{proof}
    Let $X_i = 1$ if an event occurs on the $i$-th trial in a Bernoulli. \pause
    ...
    \begin{equation}
      \label{eq:32}
      E(X) = E\lt(\sum_{i=1}^n X_i\rt) = \sum_{i=1}^n E(X_i)
    \end{equation}
...
\begin{frame}
  ...
  \begin{block}{Sketch of proof}
    You can show that variance of a single trial $Var(X_i) = E(X_i^2) - E(X_i)^2 = p - p^2 = p(1-p) = pq$. \pause
    And $Var(X)$ follows.
  \end{block}
...
\begin{frame}
  ...
  engineering majors is governed by the binomial distribution.
  \pause
  \begin{enumerate}[(a)]
  \item What is the mean of the binomial distribution governing this set of outcomes?
    \pause
  \item What is the variance?\pause
  \item Find the probability that 2 of 4 randomly selected students will be engineering majors.\pause
  \item Find the probability that at least 3 randomly selected students will be engineering majors.
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \item Find the probability that at least 3 randomly selected students will be engineering majors.\vfill
  \end{enumerate}
  \vfill
...
\begin{frame}
  ...
  We see that it can be approximated by $\mathcal{N}(\mu = np, \sigma^2 = npq)$. \pause
...
\begin{frame}
  ...
  If a binomial PMF is not too skewed, then $X\sim \text{Bin}(n,p)$ is approximately normally distributed with $\mu = np$ and $\sigma^2 = npq$.
...
%   Key assumptions:
%  \begin{itemize}
%     \item An event can occur at random and at any instant of time or any point in space
%     \item The occurrence of an event in a given time (or space) interval is statistically independent of that in any other non-overlapping interval
%     \item The probability of occurrence of an event in a small interval $\Delta t$ is proportional to $\Delta t$ and can be given by $v\Delta t$, where $v$ is the mean occurrence rate of the event
%     \item The probability of two or more occurrences in $\Delta t$ is negligible.
%     \end{itemize}
%   \begin{exampleblock}{Examples of a Poisson process}
   
%     \begin{itemize}
%     \item Breakdowns of a computer system over time
%     \item Number of accidents in an industrial facility over time
%     \item Answering calls at a telephone switchboard
%     \end{itemize}
%   \end{exampleblock}
% \end{frame}
...
%       x post scale=1.4
%       ]
%      \only<1->{\addplot+[ycomb,blue,thick,opacity=.5] {poiss(1))}; \addlegendentry{$\lambda = 1$}}
%      \only<2->{\addplot+[ycomb,red,thick,opacity=.5] {poiss(5))};}
%      \only<3->{\addlegendentry{$\lambda = 5$}}
%      \only<4->{\addplot+[ycomb,brown,thick,opacity=.5] {poiss(9))};}
%      \only<5->{\addlegendentry{$\lambda = 9$};}
%     \end{axis}
%   \end{tikzpicture}
% \end{frame}
...
% \begin{frame}
%   \frametitle{Poisson distribution}
%   If $X_t$ has a Poisson distribution with parameter $\lambda = vt$, its PMF is: \pause
%   \begin{align}
%     \label{eq:40}
%     P(X_t = x)     &= \fr{(vt)^x}{x!}e^{-vt} \quad x \ge 0  \\ \pause
%     \text{\bf OR: }\quad   P(X_t = x)    &= \fr{\la^x}{x!}e^{-\la}  \quad x \ge 0 
%   \end{align}
%   \pause
 
%   where: \pause
%   \begin{align*}
%     \la &\equiv \text{expected/mean rate per unit time or per unit area} \\ \pause
%     v   &\equiv \text{expected/mean number of occurrences in time interval $t$}
%   \end{align*}
  
%   \pause
%   \begin{block}{Mean and variance of Poisson distributed r.v.}\pause
%   Given that the expected number of events in the interval $t$ is $vt$:\pause
%   \begin{equation}
%     \label{eq:41}
%     E(X_t) = Var(X_t) = \la = vt
%   \end{equation}
...
% \begin{frame}
%   \frametitle{Relationship between binomial and Poisson distribution} \pause
...
%     \begin{equation}
%       \label{eq:43}
%       X\sim Binomial(n,p) \to X \sim Poisson(\la)
%     \end{equation} \pause
%     where $\la = np$. 
%   \end{alertblock}
%   \pause
...
%       }
%       ]
%       \only<7->{\addplot+[ycomb,opacity=.5] {binom(x,100,0.2)}; \addlegendentry{$Binomial(100,0.20)$}}
...
% \begin{frame}
%   \frametitle{Poisson distribution in practice} \pause

%   \begin{exampleblock}{Example 3: Aircraft arrival at airport} \pause Suppose small
%     aircraft arrive at a certain airport according to a Poisson process with a
%     rate $v = 8$/hr, so that the number of arrivals during a time period of $t$
%     hours is a Poisson r.v.\ with parameter $\la = 8t$. \pause
...
%     \item What are the expected value and SD of the number of small aircraft that arrive during a 90-minute period?
%     \end{enumerate}
...
%   \begin{exampleblock}{Example 3: Aircraft arrival at airport (cont.)} \pause
%     Given $\la = 8t = 8\times1 = 8$:
...
%                        &= 1 - \sum_{x =0}^9\fr{\la^x e^{-\la}}{x!} \\ \pause
...
%     \item What are the expected value and SD of the number of small aircraft that arrive during a 90-minute period?
%       \pause
%       We want to find a new $\la^*$ where $t = 1.5$ hrs and the occurrence rate is 8/hr. Thus:
%       \begin{align*}
%         \la^* &= 8 \times 1.5 = 12 \\\pause
%         \text{The new SD: }\pause \sigma^* &= \sqrt{\la^*} = \pause \sqrt{12} = \boxed{3.464}
%       \end{align*}
%     \end{enumerate}
