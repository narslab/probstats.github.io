...
\title[CEE 260/MIE 273 5A: Inference for Single Prop.]{{\normalsize CEE 260/MIE 273: Probability and Statistics in Civil Engineering} \\
Lecture 5A: Inference for Single Proportion}
\date[October 30, 2025]{\footnotesize October 30, 2025}
\author{{\bf Jimi Oke}}
...
\begin{frame}
  ...
  \item \textit{\gr Estimate} the test statistic $z$ from the sample data
  \item \textit{\gr Specify} or \textit{\gr identify} the \textbf{level of significance} ($\alpha$)
  \item \textit{\gr Define} the \textbf{region of rejection/critical region} of the null hypothesis by choosing the \textbf{critical value} $z^*$
  \item \textit{\gr Decide.} If the test statistic is in the critical region (i.e. $|z| > z^*$), reject $H_0$. If not, do not reject $H_0$ (fail to reject it)
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}
    \begin{axis}[no markers, domain=0:10, samples=100,
      ...
      xtick={-6,0,6},
      xticklabels={$z_{\frac{\alpha}{2}}$,$0$,$z_{1 - \frac{\alpha}{2}}$},
      ymax=.15,
      ytick=\empty,
      x label style={anchor=west},
      y label style={anchor=south},
  ...
  \pause
  \begin{equation}
    z^* = z_{1 - \frac{\alpha}{2}} = |z_{\frac{\alpha}{2}}|
  \end{equation}
...
\begin{frame}
  \frametitle{Example 2: Getting enough sleep (OS 5.21)}
    ...
    \item Null hypothesis: \pause $H_0: p = p_0 = .5$. \pause
    \item Alternative hypothesis: \pause $H_1: p \ne p_0$. \pause
    \item Formula for test statistic value: \pause $z = \fr{\hat p - p_0}{SE_{p}}$ \pause
    \begin{equation}
      \hat p = \fr{289}{400} = .723
    \end{equation}
    \end{enumerate}
\end{frame}
...
\draw [->, thick, orange] (axis cs: 6, 0.2) -- (axis cs: 8.5, 0.08);
...
This is a one-sided test (upper-tailed) \pause
...
\node at (axis cs: 0.8, 0.35) {\textcolor{blue}{\footnotesize in rejection region}};
...
\pause
\textbf{Decision:} Since $z = 1.875 > 1.645 = z_{\alpha}$, we \textcolor{red}{\textbf{reject}} $H_0$ at $\alpha = 0.05$ significance level.
...
\begin{frame}
  ...
  \begin{exampleblock}{Example 4: Getting enough sleep (OS 5.21)}
    ...
    \begin{enumerate}[Step 1.]
    \item Parameter of interest: \pause $p$ (proportion of students not getting enough sleep) \pause
    \item Null hypothesis: \pause $H_0: p = p_0$. \pause
    \item Alternative hypothesis: \pause $H_1: p \ne p_0$. \pause
    \item Formula for test statistic value: \pause $z = \fr{\hat p - p_0}{SE_{p}}$ \pause
    \end{enumerate}
    Notes:
    \begin{eqnarray*}
      p_0 &=& 0.50 \quad \text{(expected value)} \\
      \hat p &=& \fr{289}{400} = .723 \quad \text{(observed value---from sample)}
    \end{eqnarray*}
  \end{exampleblock}
\end{frame}
...
\pause
\textbf{Decision:} Since the p-value $= 0.03 < \alpha = 0.05$, we \textcolor{red}{\textbf{reject}} $H_0$ at the 5\% significance level.

\pause
\textbf{Conclusion:} There is sufficient evidence to conclude that the new machine produces hinges with a flaw rate greater than 0.2\%.
...
\begin{frame}
  ...
  \item Test statistic:
    ...
    \item Sample proportion with independent observations and large enough sample size (normal distribution); \pause
      ...
      \begin{equation}
     z= \fr{\hat p - p_{0}}{\sqrt{\fr{p_{0}(1-p_{0})}{n} }}
    \end{equation}
    \pause
    \end{itemize}
  ...
  \item The $p$-value is the minimum probability of a Type I error.\pause
    \begin{itemize}
    \item Upper-tailed test: p-value$ = 1- \Phi(z)$; \pause Python: \texttt{\bl norm.sf(z)} \pause
    \item Lower-tailed test: p-value $= \Phi(z)$; \pause Python: \texttt{\bl norm.cdf(z)} \pause
    \item Two-tailed test: p-value $ = 2(1- \Phi(|z|))$;\\ \pause 
