...
Lecture M6b: Inference for Two Samples}
...
\begin{frame}
  ...
  \begin{itemize}
  	...
  	\begin{equation}
  		t^* = F_T^{-1}(1 - \alpha/2, \nu)  
  	\end{equation}
  	\pause given by \texttt{tinv(1 - alpha/2, nu)} in Matlab
  	\pause
...
\begin{frame}
	\frametitle{Computing $\nu$ for difference of two means}\pause
	To find the critical $t$-value, we need to compute the degrees of freedom (df) parameter $\nu$. \pause
	
	For a single sample, $\nu = n-1$. However, when dealing with \textbf{two samples}, 
	$\nu$ given by: \pause
	
	\begin{equation}
		\label{eq:3}
		\nu = \fr{\lt(\fr{s_1^2}{n_1} + \fr{s_2^2}{n_2}\rt)^2}{\fr{\lt(s_1^2/n_1\rt)^2}{n_1 -1} + \fr{\lt(s_2^2/n_2\rt)^2}{n_2 -1}}
	\end{equation}
	\pause
...
\begin{frame}
  ...
  \begin{exampleblock}{Solution}\pause
    ...
    \begin{eqnarray*}
      \nu &=& \fr{\lt(\fr{s_1^2}{n_1} + \fr{s_2^2}{n_2}\rt)^2}{\fr{\lt(s_1^2/n_1\rt)^2}{n_1 -1} + \fr{\lt(s_2^2/n_2\rt)^2}{n_2 -1}}\\\pause
          &=& \fr{\lt( \fr{0.6241}{10} + \fr{12.8881}{10}\rt)^2}{\fr{(0.6241/10)^2}{9} + \fr{(12.8881/10)^2}{9}}  \pause
     = \fr{1.8258}{0.1850} = \pause 9.87
    \end{eqnarray*}
    \pause We round down to nearest integer. Thus, we use: $\nu = 9$.
  \end{exampleblock}
  \pause
\begin{alertblock}{Quick back-of-the-hand calculation for $\bm{\nu}$} \pause
 	A shortcut for finding $\nu$ is
 	\begin{equation}
 		\nu \approx \min(n_1 -1, n_2-1)\label{eq:10}
 	\end{equation} \pause
 	%That is, we choose the smaller d.o.f. of the two samples.{} \pause
 	In this case, we can see that since $n_1 = n_2$, we ended up with $\nu = 10 - 1 = 9$.
  \end{alertblock}
\end{frame}
...
\begin{frame}
  ...
  \begin{exampleblock}{Solution}\pause
    ...
    \begin{equation*}
      -t^* = t_{(\alpha/2, \nu)} = t_{0.025,9} = -2.262 \quad {\rd \mathtt{tinv(.025, 9)}}
    \end{equation*}
	...
	\begin{exampleblock}{Example 1: Cold-rolled vs.\ galvanized steel (cont.)}
		\begin{enumerate}[\bf Step 1.]\setcounter{enumi}{4}
			...
			\begin{eqnarray*}
				...
				z^* = z_{(1-\alpha/2)} &=& z_{0.995} = \pause 2.58 \quad (\mathtt{norminv(0.995)}) \\\pause
...
\begin{frame}
	...
	\begin{block}{Assumptions}\pause
		\begin{itemize}[<+->]
			\item $n$ independently selected pairs of observations: $E(X_i) = \mu_1, E(Y_i) = \mu_2$.
...
\begin{frame}
	...
	\begin{exampleblock}{Example 2: Intervention for ergonomic improvements (cont.)}
		\begin{enumerate}[\bf Step 1.]\setcounter{enumi}{4}\pause
			...
			\begin{eqnarray*}
				p\text{-value} &=& 2\cdot(1 - F_T(3.3,15)) \quad {\rd(\mathtt{2*tcdf(3.3, 15, "upper")})} \\\pause
