...
\begin{frame}
	...
	\begin{minipage}{.45\linewidth}
		...
		\pause
		\begin{itemize}
			\item The $F$-distribution has 2 parameters: $df_1$ and $df_2$. \pause
	...
	\begin{minipage}{.45\linewidth}
		\begin{tikzpicture}[scale=.7,
			...
			\begin{axis}[
				...
				\addplot [very thick,blue] {fdst(x,1,1)}; \addlegendentry{$df_1=1,\hphantom{00} df_2=1$}
				\addplot [very thick,orange] {fdst(x,50,50)}; \addlegendentry{$df_1=50,\hphantom{0} df_2=50$}
				\addplot [very thick,purple] {fdst(x,5,2)}; \addlegendentry{$df_1=5,\hphantom{00} df_2=2$}
...
\begin{frame}
	...
	\begin{description}
		...
		\item[Compare]
		\begin{enumerate}[(a)]
			\item Find critical value $F_{(1-\alpha, df_1, df_2)}$.\\ \pause
			If $f \le F_{(1-\alpha, df_1, df_2)}$, \textbf{fail to reject $\bm{H_0}$}  \pause \textit{OR} \\\pause
...
\begin{frame}
	...
	\begin{block}{Test statistic for one-way ANOVA}\pause
		...
		\begin{itemize}
			...
			\item When $H_0$ is not acceptable, $\mathbb{E}({\rd MSG}) > \mathbb{E}({\bl MSE}) = \sigma^2 \implies f > 1$ \pause
			\item When $H_0$ is acceptable, $f$ follows the $F$-distribution with: \pause 
			\begin{eqnarray}
				 df_1 &=& k-1 \\ \pause 
				 df_2 &=& n-k
			\end{eqnarray}
...
\begin{frame}
	...
	\begin{description}
		...
		\item[Compare]
		\begin{enumerate}[(a)]
			\item Find critical value $F_{(1-\alpha, df_1, df_2)}$\footnote{Use \texttt{\rd f.ppf(1 - alpha, df1, df2)} in Python} (also called $f^*$).\\ \pause
			If $f \le F_{(1-\alpha, df_1, df_2)}$, \textbf{\bl fail to reject $\bm{H_0}$}  \pause 
			
			If $f > F_{(1-\alpha, df_1, df_2)}$, \textbf{\rd  reject $\bm{H_0}$} 
			\textit{OR} 
			\\\pause
			\item Find $p$-value. \\ \pause
			If $p$-value $ \ge \alpha$  , \textbf{fail to reject $\bm{H_0}$} \pause

			If $p$-value $ < \alpha$, \textbf{reject $\bm{H_0}$}
		\end{enumerate}
	\end{description}
\end{frame}

\begin{frame}
	\frametitle{ANOVA F-test visualization}\pause
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\begin{axis}[
				...
				axis lines=left,
				xtick={0, 3.16},
				xticklabels={$0$, $F_{(1-\alpha, df_1, df_2)}$},
				axis on top,
				enlargelimits=upper,
				samples=150,
				xmin=0, ymin=0,
				...
				\addplot [very thick, darkcandyapplered, name path=fdist] {fdst(x,3,18)};
				
				% Rejection region (shaded in red/orange)
				\addplot [fill=darkcandyapplered!40, draw=none, domain=3.16:6, name path=rejection] {fdst(x,3,18)} \closedcycle;
				
				% Acceptance region (shaded in green)
				\addplot [fill=green!20, draw=none, domain=0:3.16] {fdst(x,3,18)} \closedcycle;
				
				% Critical value line
				\draw[thick, darkcandyapplered, dashed] (axis cs:3.16,0) -- (axis cs:3.16,0.45);
				
				% Labels
				\node[green!50!black, font=\small] at (axis cs:1.2,0.17) {\textbf{Acceptance Region}};
				\node[green!50!black, font=\small] at (axis cs:1.2,0.12) {Fail to reject $H_0$};
				
				\node[darkcandyapplered, font=\small] at (axis cs:4.3,0.15) {\textbf{Rejection Region}};
				\node[darkcandyapplered, font=\small] at (axis cs:4.3,0.11) {$\alpha = 0.05$};
				\node[darkcandyapplered, font=\small] at (axis cs:4.3,0.06) {Reject $H_0$};
				
				\node[font=\footnotesize] at (axis cs:3,0.56) {$F$-distribution};
				\node[font=\footnotesize] at (axis cs:3,0.48) {$(df_1 = k-1, df_2 = n-k)$};
	...
	\begin{alertblock}{Decision Rule}\pause
		\begin{itemize}
			\item If $f \le F_{(1-\alpha, df_1, df_2)}$: {\gr Fail to reject $H_0$} (means are equal)\pause
			\item If $f > F_{(1-\alpha, df_1, df_2)}$: {\rd Reject $H_0$} (at least one mean differs)
		\end{itemize}
	\end{alertblock}
...
\begin{frame}
	...
	\begin{exampleblock}{Example 1: Elastic moduli of alloys (cont.)}\pause
		...
		\begin{enumerate}[Step 1.]
			...
			\item Second, we find the $p$-value\footnote{In MATLAB, use: \texttt{\rd 1 - f.cdf(12.56, 2, 19)} OR \texttt{\rd f.sf(12.56, 2, 19)}}: \pause
			\begin{eqnarray*}
...
\begin{frame}
	...
	\begin{exampleblock}{Example 2: Recording tape quality}\pause
		...
		\begin{tabular}{l l l }\toprule
			\textbf{Coating} & \textbf{Observations} & \textbf{Sample Sizes} \\\midrule
			A & 10, 15, 8, 12, 15 & 5 \\
			B & 14, 18, 21, 15 & 4 \\
...
\begin{frame}
	...
	\begin{exampleblock}{Example 2: Recording tape quality (cont.)}\pause
		\begin{enumerate}[Step 1.] \setcounter{enumi}{3}
			...
			\begin{equation*}
				F_{0.95, 3, 18} = F^{-1}(0.95, 3, 18) = \rd{3.1599} \quad \text{(from Python)}
			\end{equation*}
			\pause
			
			Notes: 
			\begin{itemize}
				\item Use \texttt{\rd f.ppf(0.95, 3, 18)} to find the critical value. \pause
			 \item The $F$ curve is NOT symmetric. So, \texttt{-f.ppf(0.05, 3, 18)} will NOT give the same answer. \pause
			 \item However, the $F$-curve has the property  \pause
			 \begin{equation}
			 	F_{(\alpha,df_1,df_2)} = \fr{1}{F_{(1-\alpha, df_2, df_1)}}
			 \end{equation}\pause
			 \item So, in this case, \texttt{\rd f.ppf(0.05, 18, 3)} will give the same answer as \texttt{\rd f.ppf(0.95, 3, 18)}.
			\end{itemize}
		\end{enumerate}
	\end{exampleblock}
\end{frame}
		...
		\begin{enumerate}[Step 1.] \setcounter{enumi}{4}
			...
			\begin{tikzpicture}[scale=.7,
				...
				\begin{axis}[
					...
					\addplot [very thick,blue] {fdst(x,3,18)};% \addlegendentry{$df_1=1,\hphantom{00} df_2=1$}
					\addplot [fill=orange, domain= 3.1599:5] {fdst(x,3,18)} \closedcycle;% \addlegendentry{$df_1=50,\hphantom{0} df_2=50$}
					% \addplot [very thick,purple] {fdst(x,5,2)}; \addlegendentry{$df_1=5,\hphantom{00} df_2=2$}
			...
			Thus, $f$ lies in the critical region, and we \textbf{\og reject the null hypothesis} that the mean distortions from the four coatings are equal.
...
\section{Python}
\begin{frame}
	\frametitle{ANOVA in MATLAB/Python}\pause
	\begin{itemize}
		\item The computations in ANOVA are tedious and prone to human error.
		\item Usually ANOVA is conducted using software platforms such as MATLAB or Python.
	\end{itemize}
	...
	\begin{exampleblock}{Example 3: Investigating fuel efficiency of cars manufactured in different countries}
		...
		different countries (USA/Europe/Japan) are equal.
	\end{exampleblock}
\end{frame}
...
\begin{frame}
	...
	\begin{exampleblock}{Example 3:  Investigating fuel efficiency of cars manufactured in different countries (cont.)}
		\pause
		\begin{enumerate}[Step 1.]
			\item Examine the contents of the \texttt{6C-anova-cars.ipynb} script. Run the script.\pause
			
			\item Figure 1 is a boxplot of the MPG across the different origins.  What can you say about the variance \textit{within} each of the countries/regions, and \textit{among} them?
			
			\includegraphics[width=.4\textwidth]{example3_boxplot_python}
...
\begin{frame}
	...
	\begin{exampleblock}{Example 3:  Investigating fuel efficiency of cars manufactured in different countries (cont.)}
		\pause
		\begin{enumerate}[Step 1.]\setcounter{enumi}{2}
			\item The function \texttt{\bl scipy.stats.f\_oneway} generates  the ANOVA 
			results (though not formatted as nicely as in MATLAB---see Appendix). More detail is provided by
			\texttt{\bl statsmodels.stats.oneway.anova\_oneway}.

			\item What is the p-value for this test? \pause Based on this, do we \textbf{reject/fail to reject} the null hypothesis at any reasonable significance level?
		\pause
		\item In Figure 3, we have a notched boxplot, which denotes the confidence interval of the median.
...
\begin{frame}
	...
	\begin{itemize}\pause
		\item If after performing a one-way ANOVA, we reject the null hypothesis, naturally we might want to find out which populations significantly differ from each other
...
\begin{frame}
	...
	\begin{exampleblock}{Example 3:  Investigating fuel efficiency of cars (cont.)}
		\pause
		\begin{enumerate}[Step 1.]\setcounter{enumi}{4}
			\item Use the \texttt{\bl pairwise\_tukeyhsd} function from \texttt{\bl statsmodels.stats.multicomp} to perform multiple comparisons.  
			
			\pause
			
			\includegraphics[width=.6\textwidth]{tukey_python.png}
			
			\pause
			
			Which regions have significantly different average fuel efficiency?
		\end{enumerate}
	\end{exampleblock}
\end{frame}
