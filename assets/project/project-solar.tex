...
\fancyfoot[RO,LE]{\small Final Project: Solar} 
...
\nin{\LARGE Final Project (Option A): Solar Panel Angle {\gr }}\hfill{\bf Prof. Oke}
...
\begin{itemize}
    \item \textbf{Tilt Angle ($\theta$):} The angle between the solar panel surface and the horizontal plane (0° = flat, 90° = vertical)

\item \textbf{Insolation:} The amount of solar radiation received per unit area. Maximum when the panel is perpendicular to the sun's rays.

\item \textbf{Optimal Angle Rule of Thumb:} For fixed installations, optimal tilt angle $\approx$ latitude of location
\begin{itemize} 
...
% Power Output $\propto \cos(\theta - \theta_{\text{optimal}})$

% Where:
% • $\theta$ = panel tilt angle from horizontal
% • $\theta_{\text{optimal}}$ = angle that maximizes output for current sun position
...
\begin{compactitem}
    ...
    \item Mounting setup (e.g., cardboard, books)
...
\begin{tikzpicture}[scale=1.2]
    ...
    \node[blue, right] at (3.5,1.5) {Solar Panel};
    
    % Angle arc
    \draw[red, thick] (2.5,0) arc (0:45:0.5);
    \node[red, right] at (2.5,0.3) {$\theta$ (tilt angle)} ;
    
    % Sun
    \draw[orange, fill=yellow!80] (0,3) circle (0.3);
    \node at (0,3) {\Large $\odot$};
    
    % Sun rays
    \draw[orange, thick, dashed] (0,3) -- (3.5,1.5);
    \draw[orange, thick, dashed] (0,3) -- (3.7,1.6);
\end{tikzpicture}
\end{center}
...
\begin{compactenum}
    ...
    % \item \textbf{For Outdoor Testing:} 
    \item Work between 11am--1pm when sun angle is most consistent; face panel toward the sun
    ...
    \item Connect Drok USB tester to solar panel USB jack
    \item Keep all setup parameters constant except tilt angle
\end{compactenum}
\end{procedurebox}

\begin{warningbox}
\textbf{$\triangleright$ Critical Controls:}
\begin{compactitem}
...
\begin{compactenum} 
    \item \textbf{Start by testing the following angles:} 0°, 15°, 30°, 45°, 60°, 75°, 90°
    \item For each angle:
        \begin{itemize}
        ...
        \end{itemize}
    \item Test additional angles near your predicted optimal angle (e.g., 35°, 40°, 50°, 55°) for better resolution
    \item Aim for at least \textbf{10 data points}
\end{compactenum}
...
Use the template available on the \href{https://docs.google.com/spreadsheets/d/1xzCwlZFZiuBnjWweli-kPffb8hIk4uqAKPxgxIRLT9U/edit?usp=sharing}{Google Drive} to aid this effort.
...
Weather conditions: \underline{\hspace{3cm}} \quad Temperature: \underline{\hspace{1.5cm}}°F\\
...
\textbf{(deg)} & \textbf{Voltage (V)} & \textbf{Voltage (V)} & \textbf{Voltage (V)} & \textbf{Voltage (V)} & \\
\hline
0 & & & & & & 1.000 \\
\hline
15 & & & & & & 0.966 \\
...
• Theoretical optimal angle ($\approx$ latitude): 42.4$^\circ$
...
Plot \textbf{Angle} on x-axis vs. \textbf{Average Voltage (V)} on y-axis
\begin{compactitem}
    \item Include all data points from your experiment\footnote{To improve your results, you may also focus on the \textbf{``linear region''} of your data (angles from 20° to 60°) where the relationship is approximately linear.
...
\begin{compactitem}
    \item $y$ = voltage (V)
    \item $x$ = tilt angle (degrees)
      \item $\beta_0$ = $y$-intercept (baseline voltage)  
    \item $\beta_1$ = slope (voltage change per degree)
\end{compactitem}

\begin{calculationbox}
Report the following:
• Regression equation: y =  \_\_\_\_\_ + \_\_\_\_\_ x 
• Slope: \_\_\_\_\_ V/degree
• Intercept: \_\_\_\_\_ V
...
\item What angle produced the maximum power output in your experiment? How close is this to the theoretical optimal angle ($\approx$ 42.4° for Amherst)?

\item Interpret the slope of your linear regression. What does it tell you about how voltage changes with angle in your tested range?

\item The p-values of the model coefficients (slope and intercept) indicate their respective statistical significance.
...
\item What is your $R^2$ value for the linear model? Is the linear approximation appropriate for your selected angle range? Support with evidence from your residual plot.


\end{enumerate}
\subsection{Part 2: Experimental Design {\it (Answer Q4 AND either Q5 or Q6)} }
\begin{enumerate}[resume]
\item What were the three most significant sources of error in your experiment? How did you minimize them?

\item Calculate the absolute percent difference between your optimal angle and the theoretical optimal (latitude):
\[\text{\% difference} = \frac{|\text{experimental} - \text{theoretical}|}{\text{theoretical}} \times 100\%\]

\item Your panel produced maximum output at angle $\theta_{\text{opt}}$. Calculate the power loss 
at $\theta_{\text{opt}} \pm 10^{\circ}$:
\[\text{\% loss} = \frac{V_{\theta_{\text{opt}}} - V_{\theta_{\text{opt}} \pm 10^{\circ}}}{V_{\text{max}}} \times 100\%\]
This tells you how forgiving the optimal angle is!

\end{enumerate}

\subsection{Part 3: Engineering Applications  {\it (Answer Q7 AND any one of Q8--Q10)} }
\begin{enumerate}[resume]

\item  You're designing a solar installation on a residential roof in Amherst, MA. The roof is sloped at 25°. Based on your regression model, what percentage of potential energy production is lost compared to optimal angle? Would you recommend a mounting system to adjust the angle, or is 25° acceptable?

\item Research the typical energy gain from single-axis tracking systems (which adjust angle throughout the day) vs. fixed installations. If tracking adds \$800 to a \$4,000 system cost and increases annual energy by 25\%, calculate the payback period assuming \$0.15/kWh electricity rate and 5 kWh/day base production.
 
% \item The optimal angle changes with seasons:
% \begin{itemize}[leftmargin=*]
...
\item Flat roofs (0° panels) are common in commercial buildings despite being far from optimal. Research and explain THREE reasons why building owners might choose flat installation despite the efficiency loss.

 
\item Snow accumulation is a major concern in Massachusetts. How does panel angle affect: (a) snow shedding (self-cleaning) and (b) wind load on mounting structures.
Make a recommendation for angle considering both energy production AND structural/maintenance factors.

\end{enumerate}

\subsection{Part 4: Extra Credit}

The true relationship between voltage and tilt angle is given by: $V = V_{\text{max}} \cdot \cos(\theta - \theta_{\text{optimal}})$. Thus, repeat the regression analysis using $\cos(\theta)$ as the independent variable.
Does this model have a better fit (i.e. higher $R^2$)? Include a scatter plot and regression line for this model.
...
\begin{compactitem} 
    \item NREL (National Renewable Energy Laboratory) \href{https://pvwatts.nrel.gov/}{PVWatts Calculator}
...
\begin{calculationbox}
Optimal fixed angle $\approx$ Latitude (annual average)
Summer optimal $\approx$  Latitude - 15$^\circ$
Winter optimal $\approx$  Latitude + 15$^\circ$

Energy production  $\propto \cos(\theta_{\text{panel}} - \theta_{\text{sun}})$
